...
// Chunk takes `[]T` and returns `[][]T`, where each subarray has a max length of `chunkSize`.
...
// ChunkWhile takes `[]T` and returns `[][]T`, each of which is a contiguous chunk of items
